...
def match_algorithm(Auth_key):
	...
	if l_len > 0:
		for i in range(0, l_len, 2):
			if waiting_list[i][1] - waiting_list[i+1][1] < 1000:
...
def grade_algorithm(Auth_key):
	...
	for res in game_result:
		...
		grade_fix = int(grade_lose / res.get('taken'))

		if grade_win + grade_fix < 10000:
			commands.append({"id": winner, "grade": grade_win + grade_fix})
		else:
			commands.append({"id": winner, "grade": 9999})
		if grade_lose - grade_fix > 10:
			commands.append({"id": loser, "grade": grade_lose - grade_fix})
		else:
			commands.append({"id": loser, "grade": 10})
	change_grade = put_api(Auth_key, 'change_grade', commands)
...
print(score)
######################################################
# {'status': 'finished', 'efficiency_score': '99.8728', 
# 'accuracy_score1': '62.2222', 'accuracy_score2': '52.0389', 
# 'score': '217.0116'}

# {'status': 'finished', 'efficiency_score': '99.3709', 
# 'accuracy_score1': '30.2227', 'accuracy_score2': '62.5714', 
# 'score': '190.8496'}
