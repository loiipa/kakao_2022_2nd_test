...
def grade_algorithm(Auth_key, ans):
	...
	for res in game_result:
		...
		winner = res.get('win')
		grade_win = user_info[winner - 1].get('grade')

		loser = res.get('lose')
		grade_lose = user_info[loser - 1].get('grade')
		if ans == '1':
			if grade_win >= grade_lose:
				grade_fix = (41 - res.get('taken')) * (abs(math.log(grade_win) - math.log(grade_lose)))
			else:
				grade_fix = (41 - res.get('taken')) * math.log(abs(grade_win - grade_lose))
		if ans == '2':
			if grade_win >= grade_lose:
				grade_fix = (41 - res.get('taken')) * (abs(math.log(grade_win) - math.log(grade_lose)))
			elif grade_win < grade_lose and res.get('taken') > 10:
				grade_fix = (41 - res.get('taken')) * math.log(abs(grade_win - grade_lose))
			else:
				grade_fix = 10 - res.get('taken')

		commands.append({"id": winner, "grade": grade_win + grade_fix})
		commands.append({"id": loser, "grade": grade_lose - grade_fix})
...
print(score)
######################################################
# {'status': 'finished', 'efficiency_score': '99.974', 
# 'accuracy_score1': '9.5556', 'accuracy_score2': '52.6586', 
# 'score': '154.6361'}
# {'status': 'finished', 'efficiency_score': '99.8799',
#  'accuracy_score1': '0.3328', 'accuracy_score2': '54.0759', 
# 'score': '145.1944'}
